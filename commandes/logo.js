...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker mickey");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        ...
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by mickey-plus*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        const noArgMsg = `*_EXEMPLE *:  ${prefixe}dragonball mickey`;
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by mickey-plus*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto mickey-plus");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by mickey-plus*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "didong", categorie: "Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t* logo by mickey-plus*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
           repondre(`*exemple :* ${prefixe}didong mickey-plus`)
            return ;
        }
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by mickey-plus' }, { quoted: ms });
  ...
  {nomCom:"boom",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom mickey`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  Traitement en cour ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by mickey-plus*"})
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold mickey-plus");return;
      }
...
zokou({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena mickey-plus");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent mickey-plus");return;
      }
